...
#define CONV_KERNEL_SIZE 49
__constant__ float dc_weight[CONV_KERNEL_SIZE * CONV_KERNEL_SIZE];
            ...
            for (int kernel_row = 0; kernel_row < kernel_size; kernel_row++) // kernel_size x kernel_size filter
            {
                for (int kernel_col = 0; kernel_col < kernel_size; kernel_col++)
                {
                    ...
                    accumulator += input[(batch_idx * (input_channel * height * width)) +
                                         ...
                                         (input_row * width) +
                                         input_col] *
                                   dc_weight[(output_feature_idx * (input_channel * kernel_size * kernel_size)) +
                                              ...
                                              const int height_in, const int width_in, const int kernel_height)
{
    ...
    cudaMemcpyToSymbol(dc_weight, weight_data, output_channel * input_channel * kernel_height * kernel_height * sizeof(float));
