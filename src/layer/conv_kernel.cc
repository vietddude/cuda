...
void Conv_Kernel::forward(const Matrix &bottom)
{
    ...
    const int num_samples = n_sample; // Do not change value of these const
    const int input_channel = channel_in;
    const int output_channel = channel_out;
    const int kernel_height = height_kernel;

    KernelInterface iKernel;

    // Start timer
    GpuTimer timer;
    timer.Start();
    iKernel.forward_kernel(output_data, input_data, weight_data, bias_data,
...
void Conv_Kernel::im2col(const Vector &image, Matrix &data_col)
{
...
void Conv_Kernel::col2im(const Matrix &data_col, Vector &image)
{
    int hw_in = height_in * width_in;
    int hw_kernel = height_kernel * width_kernel;
    int hw_out = height_out * width_out;
...
void Conv_Kernel::backward(const Matrix &bottom, const Matrix &grad_top)
{
    int n_sample = bottom.cols();
    grad_weight.setZero();
    grad_bias.setZero();
...
void Conv_Kernel::update(Optimizer &opt)
{
...
std::vector<float> Conv_Kernel::get_parameters() const
{
...
void Conv_Kernel::set_parameters(const std::vector<float> &param)
{
...
std::vector<float> Conv_Kernel::get_derivatives() const
{
